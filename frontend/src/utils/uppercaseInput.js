 ...
 */
const SKIP_TYPES = new Set(['number', 'password', 'date', 'datetime-local', 'time', 'month', 'week', 'file']);

function shouldUppercase(el) {
  if (!el || typeof el.value !== 'string') return false;
  
  // Excepción para login: si el input o algún padre tiene data-no-uppercase

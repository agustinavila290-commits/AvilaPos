...
export default function ProductoPage() {
  ...
  const waConsultaText = encodeURIComponent(
    `Hola! Me interesa el producto: ${normalizar(producto.nombre_completo)} (Cód: ${producto.codigo}). ¿Está disponible?`
  )
  const waCompartirText = encodeURIComponent(
    `Mirá este repuesto en ${SITIO.nombre}: ${normalizar(producto.nombre_completo)} — ${window.location.href}`
  )
  const waConsultaUrl = `https://wa.me/${WA_NUMBER}?text=${waConsultaText}`
  const waCompartirUrl = `https://wa.me/?text=${waCompartirText}`
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {producto.categoria && (
              <span className="text-xs bg-blue-50 text-brand-blue font-medium px-2 py-0.5 rounded-full">
          ...
          <p className="text-sm text-brand-muted">Código: <span className="font-mono">{producto.codigo}</span></p>
          ...
          <div className="flex items-center gap-2">
            {sinStock
              ? <span className="flex items-center gap-1.5 text-sm text-brand-red font-medium">
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 border-2 border-brand-green text-brand-green font-semibold py-2.5 rounded-xl hover:bg-green-50 transition-colors text-sm mt-1"
          >
            {WA_SVG} Consultar por WhatsApp
          </a>

          {/* Info de envío */}
          <div className="border border-brand-border rounded-xl p-4 bg-brand-bg text-sm space-y-2 mt-1">
            ...
            <div className="flex items-center gap-2 text-brand-text">
              <span>🏪</span>
              <span>Retiro en local: {SITIO.direccion}</span>
            </div>
            <div className="flex items-center gap-2 text-brand-muted">
              <span>💬</span>
              <span>Consultá condiciones y plazos por WhatsApp</span>
            </div>
          </div>
          ...
          <div className="flex gap-2 mt-1">
            ...
            >
              {copiado
                ...
                : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg> Copiar enlace</>
              }
            </button>
            <a
              href={waCompartirUrl}
